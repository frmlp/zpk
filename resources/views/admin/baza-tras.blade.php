            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
                    <div class="navbar-nav">
                        ...
                        <a class="nav-link active" href="/admin/baza-tras"><i class="bi-table"></i> Baza tras</a>
...
<div class="modal fade" id="pathModal" tabindex="-1" aria-labelledby="pathModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        ...
        <script src="../js/admin-baza-tras.js" async></script>
        <script src="../js/admin-logout.js" async></script>

        <!-- <script src="/js/planner copy.js"></script> -->
        <!-- <script>
            ...
            function populateTable(table, paths) 
            {
                table.on('draw', function() {
                    table.rows().every(function() {
                        var rowNode = this.node();
                        var rowData = this.data();
                        $(rowNode).attr('id', paths.find(p => p.code === rowData[0]).id);
                    });
                ...
                paths.forEach(function(path) {
                    let row=table.row.add([
                        ...
                        7.65,
                        "Pętla",
                        `<button id="${path.id}" class="edit-btn btn btn-warning w-100">Edytuj</button>`,
                        `<button id="${path.id}" class="delete-btn btn btn-danger w-100">Usuń</button>`
                    ]);
            ...
            function handleNewButtonClick()
            {
                $('#newPathBtn').on('click', function() {
                    ...
                    let modalmap;
                    // $('#newPathBtn').on('shown.bs.modal', function () {
                        console.log("click");
                            if (!modalmap) {
                                modalmap = L.map('map-modal').setView([54.52, 18.49], 13);
                                    L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
                                        maxZoom: 17,
                                        attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
                                    }).addTo(modalmap);
